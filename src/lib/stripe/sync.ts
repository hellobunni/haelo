...
export function mapStripeStatusToOurs(stripeStatus: Stripe.Invoice.Status): InvoiceStatus {
  switch (stripeStatus) {
    case 'draft':
      return 'Draft';
    case 'open':
...
export async function syncStripeInvoiceToSupabase(
  ...
  console.log(`   Hosted URL: ${stripeInvoice.hosted_invoice_url || 'Not available'}`);
  ...
  const invoiceData = {
    ...
    invoice_number: stripeInvoice.number || `DRAFT-${stripeInvoice.id.slice(-8)}`,
    client_id: user.id,
    ...
    status: mapStripeStatusToOurs(stripeInvoice.status!),
