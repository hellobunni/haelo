...
export type ProjectRecord = {
    ...
    description: string;
    clientEmail: string;
    status: "Discovery" | "Design" | "Development" | "Testing" | "Launch" | "Completed";
    progress: number;
    startDate: string;
  ...
  const MOCK_PROJECTS: ProjectRecord[] = [
    // Sarah's projects
    {
      ...
      description: "Complete redesign of the online store with improved UX and mobile optimization",
    ...
    // Michael's projects
    {
      ...
      description: "Building a full-stack SaaS application with user authentication and analytics",
  ...
  export async function getProjectsByEmail(email: string): Promise<ProjectRecord[]> {
  ...
  export async function getDocumentsByEmail(email: string): Promise<DocumentRecord[]> {
  ...
  export async function updateDocumentStatus(id: string, status: "Pending" | "Viewed" | "Signed"): Promise<DocumentRecord | null> {
