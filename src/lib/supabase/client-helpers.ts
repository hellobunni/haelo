...
export async function getCurrentUserClient(): Promise<User | null> {
  try {
    ...
    try {
      ...
      console.warn('getUser() timed out, falling back to getSession():', timeoutError)
    ...
    console.log('getCurrentUserClient: Auth user found:', authUser.id, authUser.email)
    ...
    console.log('getCurrentUserClient: Profile result:', { profile: !!profile, error: profileError })
    ...
    console.log('getCurrentUserClient: Returning user:', profile.email, profile.role)
    return {
      id: profile.id,
      email: profile.email,
...
export async function getUserFromSession(userId: string): Promise<User | null> {
  try {
    ...
    console.log('getUserFromSession: Profile result:', { profile: !!profile, error: profileError })
    ...
    console.log('getUserFromSession: Returning user:', profile.email, profile.role)
    return {
      id: profile.id,
      email: profile.email,
      password: '',
