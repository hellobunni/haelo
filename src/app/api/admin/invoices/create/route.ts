...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('📦 Full Stripe invoice data:', {
      ...
      customer: typeof fullInvoice.customer === 'object' ? {
      ...
      due_date: fullInvoice.due_date ? new Date(fullInvoice.due_date * 1000).toISOString() : null,
    ...
    // Sync to Supabase
    const syncedInvoice = await syncStripeInvoiceToSupabase(fullInvoice, supabase);
