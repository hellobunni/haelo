...
export default function DocumentsTab() {
  const [documents, setDocuments] = useState<(DocumentRecord & { clientName: string })[]>([]);
  const [loading, setLoading] = useState(true);
  const [pdfViewerOpen, setPdfViewerOpen] = useState(false);
  const [selectedDocumentPdf, setSelectedDocumentPdf] = useState<{ url: string; title: string } | null>(null);
  const [uploadDialogOpen, setUploadDialogOpen] = useState(false);
  const [editDialogOpen, setEditDialogOpen] = useState(false);
  const [selectedDocument, setSelectedDocument] = useState<(DocumentRecord & { clientName: string }) | null>(null);
  ...
  const handlePdfUpload = (file: File, url: string) => {
    if (selectedDocument) {
      ...
      console.log(`📄 PDF uploaded for document ${selectedDocument.documentName}:`, url);
    }
  };

  const handleSaveDocument = (updatedDocument: DocumentRecord & { clientName: string }) => {
    const updatedDocuments = documents.map((doc) =>
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Button onClick={() => { setSelectedDocument(null); setUploadDialogOpen(true); }} size="sm">
      ...
      <PdfUploadDialog
        ...
        title={selectedDocument ? `Upload PDF for ${selectedDocument.documentName}` : "Upload New Document"}
